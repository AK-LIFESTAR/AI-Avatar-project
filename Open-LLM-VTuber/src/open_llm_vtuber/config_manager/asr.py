...
class OpenAIApiASRConfig(I18nMixin):
    """Configuration for OpenAI Whisper API ASR."""

    api_key: str = Field(..., alias="api_key")
    model: str = Field("whisper-1", alias="model")
    lang: Optional[str] = Field("en", alias="lang")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_key": Description(
            en="API key for OpenAI Whisper API", zh="OpenAI Whisper API 的 API 密钥"
        ),
        "model": Description(
            en="Name of the OpenAI Whisper model to use",
            zh="要使用的 OpenAI Whisper 模型名称",
        ),
        "lang": Description(
            en="Language code (leave empty for auto-detect)",
            zh="语言代码（留空以自动检测）",
...
class ASRConfig(I18nMixin):
    ...
    openai_api_asr: Optional[OpenAIApiASRConfig] = Field(
        None, alias="openai_api_asr"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "openai_api_asr": Description(
            en="Configuration for OpenAI Whisper API ASR", zh="OpenAI Whisper API ASR 配置"
        ),
