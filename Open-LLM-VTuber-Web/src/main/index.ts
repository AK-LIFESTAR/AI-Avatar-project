...
function setupIPC(): void {
  ...
  ipcMain.on("window-close", () => {
    const window = windowManager.getWindow();
    if (window) {
      // For energy-saving: closing the window should fully quit the app,
      // so the backend we spawned will also be stopped in before-quit.
      app.quit();
    }
  });
...
app.whenReady().then(() => {
  ...
  window.on("close", (event) => {
    if (!isQuitting) {
      ...
      // On macOS, users often click the red close button. We quit instead of hiding
      // to ensure backend doesn't keep running when the user "closes" the app.
      app.quit();
    }
    return false;
  });
  ...
  // Start the backend only when the user launches the desktop app.
  // This avoids idle energy usage when the app isn't in use.
  backendManager.startIfNeeded().catch((err) => {
