...
export function useMediaCapture() {
  ...
  const captureFrame = useCallback(async (stream: MediaStream | null, source: 'camera' | 'screen') => {
    ...
    const tryGrab = async (): Promise<ImageBitmap> => {
    ...
    try {
      ...
      try {
        bitmap = await tryGrab();
      } catch (firstError) {
        // Streams can take a short time to become frame-ready right after enabling.
        // Retry once to avoid sending empty image sets, which causes the LLM to say it can't see.
        console.warn(`First ${source} grabFrame() failed, retrying once...`, firstError);
      ...
      // Don't spam errors if capture fails intermittently.
      // Show a toast only for persistent failures.
      toaster.create({
        title: `${t('error.failedCapture', { source })}`,
