    ...
    >
      ...
      <Box>
        ...
        <Text color="whiteAlpha.500" fontSize="xs" lineHeight="1.4">
          Click to share screen.<br />
          AI will see your display.
      </Text>
      </Box>
    </Box>
  );
...
function VideoStream({
  ...
  return (
    <video
      ...
      muted
      style={{
        ...sidebarStyles.screenPanel.video,
...
function ScreenPanel(): JSX.Element {
  ...
  // Track if screen stream is ready
  const [isReady, setIsReady] = useState(false);

  // Check if screen is producing frames
  useEffect(() => {
    if (isStreaming) {
  ...
  return (
    ...
    >
      ...
      <Tooltip
        showArrow
        content={isStreaming ? 'Click to stop sharing' : 'Click to share screen for AI vision'}
      ...
      >
        <Box
          ...
          _hover={{
            borderColor: isStreaming ? colors.success : colors.accentSecondary,
      ...
      {isStreaming && isReady && (
        <Box mt={2}>
          <Text 
            ...
            fontStyle="italic"
          >
            AI can see your screen. Ask about it!
          </Text>
        </Box>
      )}
    </Box>
